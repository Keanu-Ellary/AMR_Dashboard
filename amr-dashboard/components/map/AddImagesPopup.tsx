...
export default function AddImagesPopup({
  isOpen,
  onClose,
  initialSite,
  onRefresh,
      ...
      const fetchSites = async () => {
        ...
        if (response.ok) {
          ...
          const uniqueSites = Array.from(
            new Map(
              data.sites.map((s: SiteData) => [s.geoLocName, s]),
            ).values(),
          ) as SiteData[];
          setSites(uniqueSites);
          if (initialSite) {
            const site = uniqueSites.find(
              (s: SiteData) => s.geoLocName === initialSite.geoLocName,
            );
    ...
    <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
            ...
            {!selectedSite ? (
                ...
                {searchQuery && (
                  <div className="absolute top-full left-0 right-0 mt-2 max-h-48 overflow-y-auto bg-white border border-border rounded-xl shadow-xl z-50 overflow-hidden">
                    {filteredSites.length > 0 ? (
                      ...
                      <div className="px-4 py-4 text-center text-xs font-bold text-gray-300 uppercase tracking-widest">
                        No target found
                      </div>
                    )}
                  </div>
              ...
              <div className="flex items-center justify-between p-4 bg-brand-50 border border-brand-100 rounded-xl">
                <div>
                  ...
                  <div className="text-[10px] text-brand-600 font-bold uppercase tracking-widest mt-0.5">
                    {selectedSite.geoLocName}
                  </div>
                </div>
                <button
                  onClick={() => setSelectedSite(null)}
                  className="text-[10px] font-bold text-brand-700 hover:text-brand-900 uppercase tracking-widest bg-white/50 px-3 py-1.5 rounded-lg border border-brand-200 transition-all"
                >
                  Change Target
                </button>
              </div>
          ...
          <section
            className={clsx(
              ...
              !selectedSite && "opacity-30 grayscale pointer-events-none",
            )}
          >
            <h2 className="text-[10px] font-bold uppercase tracking-widest text-brand-600 border-b border-brand-100 pb-2 flex items-center gap-2">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                    {imageBase64.length} Assets Staged
                  </span>
                  <button
                    onClick={() => setImageBase64([])}
                    className="text-[10px] font-bold text-risk-high uppercase tracking-widest hover:underline"
                  >
                    Clear Stage
                  </button>
                </div>
                <div className="grid grid-cols-4 sm:grid-cols-6 gap-3">
                  {imageBase64.map((src, idx) => (
                    <div
                      key={idx}
                      className="relative group aspect-square rounded-lg overflow-hidden bg-gray-100 border border-border shadow-subtle"
                    >
                      <img
                        src={src}
                        alt="Evidence"
        ...
        <div className="p-6 bg-gray-50 border-t border-border">
          ...
          >
            {isUploading ? (
              <span className="animate-pulse">Ingesting Matrix...</span>
            ) : (
              <>
                <Check size={16} /> Execute Batch Upload
              </>
            )}
          </button>
        </div>
