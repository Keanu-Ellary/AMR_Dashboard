...
describe("Advanced Statistics and Trend Integration", () => {
  beforeAll(async () => {
    await prisma.$executeRawUnsafe(
      `TRUNCATE TABLE "SiteData", "AdminUser" CASCADE;`,
    );
  ...
  it("should calculate improving/worsening trends over time correctly", async () => {
    const req = new Request(
      "http://localhost:3000/api/statistics/trendOverTime",
    );
  ...
  it("should return formatted comparison data and Pearson correlations", async () => {
    const req = new Request(
      "http://localhost:3000/api/statistics/comparison?sites=Site A,Site B&dateRange=30days&metrics=ph,temperature,wqi",
    );
