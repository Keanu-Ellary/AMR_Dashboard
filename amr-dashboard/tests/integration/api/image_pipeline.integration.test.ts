...
describe("Image Upload and AI Pipeline Integration", () => {
  ...
  it("should process a batch of images, hit the AI Lambda, and store in MinIO and DB", async () => {
    ...
    global.fetch = jest.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        if (input.toString().includes("8080/2015-03-31")) {
          return new Response(
            JSON.stringify({ results: [[10, 10, 50, 50, 0.95, 0]] }),
            { status: 200 },
          );
        }
    ...
    const req = new Request(`http://localhost:3000/api/site/${siteId}/photos`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    const res = await photosPost(req, {
      params: Promise.resolve({ id: siteId.toString() }),
    });
