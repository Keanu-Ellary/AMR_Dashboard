...
jest.mock("next/headers", () => ({
  cookies: jest.fn().mockImplementation(async () => ({
    ...
    get: jest
      .fn()
      .mockReturnValue({ value: JSON.stringify({ token: global.adminToken }) }),
    delete: jest.fn(),
  })),
}));
...
Sample B,Lake,2026-05-16,Test Lake B,-26.1,28.0,vanA,I,Genomic,6.8,18.0,200`;
    ...
    const file = new File([csvContent], "test_import.csv", {
      type: "text/csv",
    });
    const formData = new FormData();
    formData.append("file", file);

    const req = new Request("http://localhost:3000/api/site/multiple", {
      method: "POST",
    ...
    const sites = await prisma.siteData.findMany({
      where: { geoLocName: { in: ["Test River A", "Test Lake B"] } },
    });
  ...
  it("should filter map data by bounding box, date, and zone", async () => {
    ...
    const searchUrl =
      "http://localhost:3000/api/site?minLat=-26.0&maxLat=-25.0&minLong=28.1&maxLong=28.5";
  ...
  it("should bulk delete sites by Date Range and log BULK_DELETE", async () => {
    const req = new Request("http://localhost:3000/api/site/bulk", {
      method: "DELETE",
      headers: {
