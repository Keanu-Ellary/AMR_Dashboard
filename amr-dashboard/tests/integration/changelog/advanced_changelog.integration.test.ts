...
jest.mock("next/headers", () => ({
  cookies: jest.fn().mockImplementation(async () => ({
    get: jest.fn().mockReturnValue({ value: JSON.stringify({ token: global.adminToken, isAdmin: true }) }),
  })),
}));
...
describe("Advanced Changelog and Bulk Undo Integration", () => {
  ...
  beforeAll(async () => {
    await prisma.$executeRawUnsafe(`TRUNCATE TABLE "SiteData", "ChangeLog", "AdminUser" CASCADE;`);
  ...
  it("should paginate and filter changelog records correctly", async () => {
    ...
    for (let i = 0; i < 15; i++) {
      await prisma.changeLog.create({
        data: {
          entityType: "SiteData", entityId: i, action: i % 2 === 0 ? "CREATE" : "UPDATE",
          changedBy: adminId, newData: "{}"
        }
      });
    }

    const req = new Request("http://localhost/api/changelog?page=2&limit=5&action=CREATE");
  ...
  it("should successfully undo a BULK_CREATE action", async () => {
    ...
    const s1 = await prisma.siteData.create({
      data: { sampleName: "B1", isolationSource: "W", collectionDate: new Date(), geoLocName: "X", dangerZone: "red", sampleAnalysisType: "G", amrResGenes: "", predictedSir: "", adminId }
    });
    const s2 = await prisma.siteData.create({
      data: { sampleName: "B2", isolationSource: "W", collectionDate: new Date(), geoLocName: "X", dangerZone: "red", sampleAnalysisType: "G", amrResGenes: "", predictedSir: "", adminId }
    });

    // 2. Create the associated BULK_CREATE changelog
    const bulkLog = await prisma.changeLog.create({
      data: {
        entityType: "SiteData", entityId: 0, action: "BULK_CREATE", changedBy: adminId,
        newData: JSON.stringify([{ id: s1.id }, { id: s2.id }])
      }
    });

    // 3. Trigger Undo
    const req = new Request("http://localhost/api/changelog/undo", {
      method: "POST",
    ...
    const sites = await prisma.siteData.findMany({ where: { id: { in: [s1.id, s2.id] } } });
    expect(sites).toHaveLength(0);

    const undoLog = await prisma.changeLog.findFirst({ where: { action: "UNDO_BULK_CREATE" } });
  ...
  it("should successfully undo a BULK_DELETE action", async () => {
    ...
    const deletedPayload = [
      { sampleName: "Restored 1", isolationSource: "W", geoLocName: "X", dangerZone: "green", sampleAnalysisType: "G", amrResGenes: "", predictedSir: "" },
      { sampleName: "Restored 2", isolationSource: "W", geoLocName: "X", dangerZone: "yellow", sampleAnalysisType: "G", amrResGenes: "", predictedSir: "" }
    ];

    const bulkLog = await prisma.changeLog.create({
      data: {
        entityType: "SiteData", entityId: 0, action: "BULK_DELETE", changedBy: adminId,
    ...
    const restoredSites = await prisma.siteData.findMany({ where: { sampleName: { in: ["Restored 1", "Restored 2"] } } });
